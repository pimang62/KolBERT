...
class SimpleMiner(HardNegativeMiner):
    ...
    def __init__(
        ...
        self.model = AutoModel.from_pretrained(hub_model)
    ...
    def encode(self, texts, batch_size=32):
        ...
        with torch.no_grad():
            ...
            for i in range(0, len(texts), batch_size):
                ...
                # Assumes we take the mean of the last hidden states as the sentence embedding
    ...
    def build_index(
        ...
        self.voyager_index = Index(
            Space.Cosine,
            num_dimensions=embeds.shape[1], ## self.model.get_sentence_embedding_dimension(),
    ...
    def query_index(self, query, top_k=110) -> tuple:
    ...
    def _mine(
        ...
        q_emb = self.encode(query) ## self.model.encode(query)
        ...
        query_results = [self.corpus_map[x] for x in query_results[0].flatten().tolist()]
    ...
    def _batch_mine(
        ...
        query_embeddings = self.encode(queries, show_progress_bar=True) ## self.model.encode(queries, show_progress_bar=True)
